...
namespace DealRobot
{
    partial class LogicRobot
    {
        ...
        public event Action RobotDisConnect_event;//机器人断开链接
            ...
            try
            {
                ...
                switch (Head)
                {
                    ...
                    case "P"://点位
                        try
                        {
                            int i = int.Parse(cmd[1].Trim());//点位序号
                            Point2D p = new Point2D();
                            p.DblValue1 = Convert.ToDouble(cmd[2]);
                            p.DblValue2 = Convert.ToDouble(cmd[3]);
                            ComValue.C_I.RobotValue_L[i] = p;
                        }
                        catch (Exception ex)
                        {
